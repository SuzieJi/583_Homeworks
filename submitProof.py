...
def generate_primes(num_primes):
    ...
    #TODO YOUR CODE HERE
    limit = 100000  # 8192th prime is 84017 左右，100k 足够
    sieve = [True] * (limit + 1)
...
def sign_challenge(challenge):
    ...
    eth_message = eth_account.messages.encode_defunct(text=challenge)
    eth_sig_obj = acct.sign_message(eth_message)
...
def send_signed_msg(proof, random_leaf):
    ...
    tx_dict = contract.functions.submit(proof, random_leaf).buildTransaction({
        'from': acct.address,
    ...
    raw = w3.eth.send_raw_transaction(signed.rawTransaction)
    tx_hash = raw.hex()
